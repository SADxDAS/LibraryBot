...
**Data layer:** Despite being named `GoogleSheetsService`, the service now uses EF Core + PostgreSQL exclusively. `Data/AppDbContext.cs` defines the DB context; `Data/DbModels.cs` has the entities (`DbBook`, `DbBorrowing`, `DbPendingRequest`, `DbExchangeLog`). **Books are addressed by their stable `DbBook.Id`** (carried in callback data and exposed at `COL_CATALOG_ID` in the `GetBooksAsync` row shape; mutation methods take `bookId` and use `db.Books.FindAsync`). The legacy session/request field names ending in `RowIndex` (`CatalogRowIndex`, `EditRowIndex`, `OldBookRowIndex`, `LibraryBookRowIndex`) now carry that **Id**, not a position. Only `ExtendBorrowingAsync` still uses positional `rowIndex - 2` on borrowings — safe because borrowings are append-only (returns set `ReturnDate`, rows are never deleted, so positions never shift).
